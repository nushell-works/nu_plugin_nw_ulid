...
mod performance_tests {
    ...
    fn test_single_ulid_generation_performance() {
        ...
        println!("Single ULID generation: {:.2} µs per operation", per_operation.as_nanos() as f64 / 1000.0);
        
        // Performance target: should be under 5 microseconds per operation (debug mode)
        assert!(per_operation.as_nanos() < 5_000, 
    ...
    fn test_bulk_generation_performance() {
        ...
        println!("Bulk ULID generation: {:.2} µs per operation", per_operation.as_nanos() as f64 / 1000.0);
        
        assert_eq!(ulids.len(), batch_size);
        
        // Bulk generation should be faster than individual generation (debug mode tolerance)
        assert!(per_operation.as_nanos() < 10_000, 
    ...
    fn test_validation_performance() {
        ...
        println!("ULID validation: {:.2} ns per operation", per_operation.as_nanos() as f64);
        
        // Validation should be very fast
        assert!(per_operation.as_nanos() < 1_000, 
    ...
    fn test_parsing_performance() {
        ...
        println!("ULID parsing: {:.2} µs per operation", per_operation.as_nanos() as f64 / 1000.0);
        
        // Parsing should be reasonably fast
        assert!(per_operation.as_nanos() < 2_000, 
    ...
    fn test_timestamp_extraction_performance() {
        ...
        for ulid in &ulids {
            let _timestamp = UlidEngine::extract_timestamp(ulid).expect("Extraction should succeed");
        }
        
        let duration = start.elapsed();
        let per_operation = duration / ulids.len() as u32;
        
        println!("Timestamp extraction: {:.2} ns per operation", per_operation.as_nanos() as f64);
    ...
    fn test_memory_efficiency() {
        ...
        let generation_overhead = after_generation - initial_memory;
        ...
        assert!(string_overhead < expected_string_memory * 2, 
                "String memory usage too high: {} bytes (expected ~{})", 
                string_overhead, expected_string_memory);
    ...
    fn test_concurrent_performance() {
        ...
        println!("Concurrent ULID generation ({} threads): {:.2} µs per operation", 
                 thread_count, per_operation.as_nanos() as f64 / 1000.0);
...
mod reference_comparison {
    ...
    fn test_comparison_with_ulid_crate() {
        ...
        assert!(ratio < 5.0, 
                "Our implementation is too slow compared to reference: {:.2}x slower", ratio);
    }
    ...
    fn test_validation_comparison() {
        ...
        assert!(ratio < 3.0, 
                "Our validation is too slow compared to reference: {:.2}x slower", ratio);
    }
}