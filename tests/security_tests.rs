...
mod security_tests {
    ...
    fn test_cryptographic_randomness_quality() {
        ...
        for ulid in &ulids {
            assert!(unique_ulids.insert(ulid.clone()), "ULID collision detected: {}", ulid);
        }
    ...
    fn test_randomness_distribution(ulids: &[String]) {
        ...
        for (ch, count) in char_frequency {
            ...
            assert!(
                ...
                ch, count, expected_frequency as usize, tolerance as usize
            );
        }
    }
    ...
    fn test_no_sequential_patterns(ulids: &[String]) {
        ...
        for i in 1..ulids.len().min(100) { // Check first 100 pairs
            let prev_randomness = &ulids[i-1][10..];
            let curr_randomness = &ulids[i][10..];
            
            assert_ne!(
                prev_randomness, curr_randomness,
    ...
    fn test_input_validation_security() {
        ...
        for input in malicious_inputs {
            ...
            assert!(!is_valid, "Malicious input '{}' should not validate as ULID", 
                   input.chars().map(|c| if c.is_control() { '�' } else { c }).collect::<String>());
            
            // Parsing should return error without panicking
            let parse_result = UlidEngine::parse(input);
            assert!(parse_result.is_err(), "Malicious input should not parse successfully");
        }
    }
    ...
    fn test_timing_attack_resistance() {
        ...
        assert!(ratio < 100.0, "Extreme timing variance detected: {:.2}x difference", ratio);
        
        // Log timing information for manual analysis
        println!("Timing analysis: valid={:?}, invalid_prefix={:?}, invalid_start={:?}, ratio={:.2}x", 
    ...
    fn test_bulk_operation_limits() {
        ...
        assert!(result.is_err(), "Should reject bulk generation beyond limit");
        ...
        if let Ok(ulids) = result {
            ...
            for ulid in ulids {
                ...
                assert!(UlidEngine::validate(&ulid_str), "Generated ULID should be valid");
                assert!(unique_set.insert(ulid_str), "Generated ULIDs should be unique");
            }
        }
    }
    ...
    fn test_error_information_leakage() {
        ...
        for input in test_inputs {
            if let Err(error) = UlidEngine::parse(input) {
                ...
                assert!(!error_msg.contains("/"), "Error should not contain file paths: {}", error_msg);
                assert!(!error_msg.contains("\\"), "Error should not contain file paths: {}", error_msg);
                assert!(!error_msg.contains("0x"), "Error should not contain memory addresses: {}", error_msg);
                assert!(!error_msg.contains("panic"), "Error should not contain panic details: {}", error_msg);
                
                // Error message should be user-friendly
                assert!(!error_msg.is_empty(), "Error message should not be empty");
                assert!(error_msg.len() < 200, "Error message should be reasonably short");
            }
        }
    }
    ...
    fn test_concurrent_safety() {
        ...
        assert_eq!(unique_ulids.len(), all_ulids.len(), "All ULIDs should be unique across threads");
    }
}