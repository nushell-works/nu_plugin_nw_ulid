...
fn benchmark_ulid_generation(c: &mut Criterion) {
    ...
    for size in [10, 100, 1000, 10000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("generate_bulk", size), size, |b, &size| {
            b.iter(|| {
                let ulids = UlidEngine::generate_bulk(size).expect("Bulk generation should succeed");
                black_box(ulids)
            })
        });
    }
    ...
    group.bench_function("generate_with_timestamp", |b| {
        ...
        b.iter(|| {
            let ulid = UlidEngine::generate_with_timestamp(timestamp).expect("Generation should succeed");
            black_box(ulid)
        })
    });
...
fn benchmark_ulid_parsing(c: &mut Criterion) {
    ...
    group.bench_function("extract_timestamp", |b| {
        let ulid = &valid_ulids[0];
        b.iter(|| {
            let timestamp = UlidEngine::extract_timestamp(black_box(ulid)).expect("Extraction should succeed");
            black_box(timestamp)
        })
    });
